...
class DGraphNode:
    ...
    def _add_forward_link(self, target_node_id: int):
        ...
        if target_node_id != self.internal_id:  # Prevent self-loops
            if self.forward_links.add(target_node_id):
    ...
    def _add_back_link(self, source_node_id: int):
        ...
        if source_node_id != self.internal_id:
            if self.back_links.add(source_node_id):
                self.graph._mark_dirty(self.internal_id)  # Mark dirty only if changed
...
class DGraph:
    ...
    def __post_init__(self):
        ...
        if self.nodes and not self._key_id_to_node_id:
            print(
                "Warning: Rebuilding key index from existing nodes (should not happen with default_factory)"
            )
            for node_id, node in self.nodes.items():
                if node.key_internal_id != -1:
                    ...
                    if node.key_internal_id not in self._key_id_to_node_id:
                        ...
                        print(
                            f"Warning: Key ID {node.key_internal_id} collision during index rebuild."
                        )
    ...
    def add_node(self, key: DGraphNodeKey) -> DGraphNode:
        ...
        if check_key_id is not None and check_key_id in self._key_id_to_node_id:
            ...
            if existing_node_id in self.nodes:
                raise ValueError(
                    f"Key {key} is already associated with node {existing_node_id} in this graph."
                )
            else:
...
class DGraphWrap:
    ...
    def get_or_add_node(self, key: DGraphNodeKey) -> DGraphNode:
        """
    ...
    def connect_nodes_by_key(self, source_key: DGraphNodeKey, target_key: DGraphNodeKey) -> bool:
        """
    ...
    def disconnect_nodes_by_key(self, source_key: DGraphNodeKey, target_key: DGraphNodeKey) -> bool:
        """
...
if __name__ == "__main__":
    ...
    node_g1 = wrap.get_or_add_node(key_g1)
    node_p1 = wrap.get_or_add_node(key_p1)
    node_c1 = wrap.get_or_add_node(key_c1)
    node_c2 = wrap.get_or_add_node(key_c2)
    ...
    wrap.connect_nodes_by_key(key_c2, key_p1)
    wrap.connect_nodes_by_key(key_p1, key_g1)
    ...
    print(f"Node for P2 exists before get? {graph.get_node_by_key(key_p2)}")
    ...
    print(
        f"Node P2 correctly added to graph._key_id_to_node_id? {graph._key_id_to_node_id.get(key_p2_id) == node_p2.internal_id}"
    )
    # --- End Test ---

    print("\nGraph after connections and auto-add:")
    for node_id in list(graph.nodes.keys()):
        print(graph.get_node(node_id))
    print(f"\nDirty nodes after connections: {graph.dirty_node_ids}")
    print(f"Graph has cycle? {graph.has_cycle()}")

    # Introduce cycle
    print("\nIntroducing cycle G1 -> C1...")
    wrap.connect_nodes_by_key(key_g1, key_c1)
    print(f"Graph has cycle? {graph.has_cycle()}")

    # Remove cycle
    print("\nRemoving cycle G1 -> C1...")
    wrap.disconnect_nodes_by_key(key_g1, key_c1)
    print(f"Graph has cycle? {graph.has_cycle()}")
    ...
    print("\n--- Copied Graph ---")
    for node_id in list(graph_copy.nodes.keys()):
        print(graph_copy.get_node(node_id))
    ...
    print(
        f"Nodes dict shared? (graph.nodes is graph_copy.nodes): {graph.nodes is graph_copy.nodes}"
    )

    # Verify a node object is a different object
    print(
        f"Node C1 shared? (graph.nodes[{node_c1.internal_id}] is graph_copy.nodes[{node_c1.internal_id}]): {graph.nodes[node_c1.internal_id] is graph_copy.nodes[node_c1.internal_id]}"
    )

    # Verify the copied node points back to the copied graph
    print(
        f"Copied Node C1 points to copied graph? (copied_node_c1.graph is graph_copy): {copied_node_c1.graph is graph_copy}"
    )

    # Verify modifying copy doesn't affect original
    print("\nModifying copy (disconnect C2 -> P1)...")
    wrap_copy = DGraphWrap(graph_copy)
    wrap_copy.disconnect_nodes_by_key(key_c2, key_p1)
    ...
    print(
        f"Key object for C2 exists after GC?: {graph.group.get_key_from_id(key_id_c2) is not None}"
    )
    ...
    print(
        f"Key C2 still in strong map?: {key_c2_ref in graph.group._key_to_internal_id}"
    )  # Should be False if key_c2_ref is the original key
